...
namespace rd {
	...
	void Sprite::render() const
	{
		...
		model = glm::translate(model, glm::vec3(m_position, 0.f)); // 5 - change pos
		model = glm::translate(model, glm::vec3(0.5f * m_size.x, -0.5f * m_size.y, 0.f)); // 4 - move back
		model = glm::rotate(model, glm::radians(m_rotation), glm::vec3(0.f, 0.f, 1.f)); // 3 - rotate
		model = glm::translate(model, glm::vec3(-0.5f * m_size.x, -0.5f * m_size.y, 0.f)); // 2 - move to centre
	...
	void Sprite::setPosition(const glm::vec2& posotion)
	{
		m_position = posotion;
	}
